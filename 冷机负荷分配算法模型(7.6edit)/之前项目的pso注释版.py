...
from team_work.basic_class.chiller_class import Chiller
from team_work.basic_class.chiller_group_class import Chiller_group
from team_work.basic_class.coolingtower_class import Cooling_tower
from team_work.basic_class.pump_class import Decdwp_pump_group
from team_work.basic_class.tools import conn_000012
from team_work.basic_class.ALi_Class import Local_2_AliCloud
import numpy as np
import pandas as pd
import time
...
def get_system_sql_and_num_dict(system_id, now_minus_10, now_minus_5, dech_num=3,
                                decdwp_num=4, dect_num=3, decowp_num=1, deth_num=1):
    system_sql_dict = {}
...
class Optimizer_moudle:
    ...
    def get_system_dict(self):
        ...
        # 经过一整个循环 只有'dech','decdwp','dect','decowp','deth' 这些系统都Pass，judge值才会是'Pass'，否则是'Fall'
    ...
    def get_current_data(self):
        ...
        if judge == 'Fall':
            ...
            system_frame['decdwp_power_active_sum_predict'] = self.Decdwp_moudle.\
                power_predict(system_frame[['decdwp_freq_per']].values,int(system_frame['decdwp_open']))
            ...
            for i in self.Chiller_group_moudle.chiller_list:
                name = i.device_name
                tmp = system_frame[[
                    '{}_Qcond'.format(name), 'dech_flow_instantaneous_2_sum', 'decowp01_temp_2']]
            ...
            system_frame['tower_out_temp_predict'] = self.Cooling_tower_moudle.\
                out_temp_fan_off_predict(system_frame[[
                    # 'dech_Qcond_sum',
    ...
    def get_problem(self, system_frame):
        ...
        def evalVars(Vars):  # 定义目标函数（含约束）
            ...
            # 获取模拟流量
            flow = self.Decdwp_moudle.flow_instantaneous_predict(np.array([[decdwp_freq
                                                                               # , delta_pr
                                                                            ]]),
                                                                 int(decdwp_num))/decdwp_delta_pr_per
            # 计算模拟温差设定
            delta_temp = Qcond * 3.6 / (cpw * flow)
            delta_temp_set = float(delta_temp - temp_fix_delta_temp)
            # 计算出塔水温
            tower_temp_out = float(self.Cooling_tower_moudle.out_temp_fan_off_predict(
                np.array([[
                    # Qcond,
            ...
            # 约束需要根据项目进行更改
            CV = np.array([3 - delta_temp_set, delta_temp_set - 5.5,# 温差控制约束
                           15 - tower_temp_out, tower_temp_out - 30, # 设置出塔水温约束
                           max_chiller_pcond - 130, # 设置pcond最大值约束
                           max(2-apporoach,apporoach-12), #设置冷却塔逼近度约束
                           abs(delta_temp_set - delta_temp_predict_set) - 0.6,
                           abs(tower_temp_out - current_tower_temp_out) - 1.5,# 设置出塔水温变化幅度约束
    ...
    def solve_problem(self,evalVars,prior_array):
        ...
        algorithm = ea.soea_SEGA_templet(problem,
                                         ...
                                         trappedValue=0.01,  # 单目标优化陷入停滞的判断阈值。
    ...
    def solve(self, up_load = 0):
        ...
        if judge == 'Fall':
            ...
            if up_load == 1:
                logging.info('上传寻优参数')
                try:
                    self.Up_load_moudle.upload_decdwp_num(sys.argv[1:],int(decdwp_num_set))
                    self.Up_load_moudle.upload_tower_num(sys.argv[1:],int(dect_num_set))
                    self.Up_load_moudle.upload_tower_out_temp(sys.argv[1:],int(tower_temp_out_set*10))
                    self.Up_load_moudle.upload_temp_delta(sys.argv[1:],int(delta_temp_set*10))
                except:
                    logging.info('上传报错')
            else:
                logging.info('不上传参数')
...
#                           minute=0),hour=12)
    ...
    #                     minute=0)
    # time.sleep(60*30)
    log_frame_all, system_frame_all = simulate_solve_now(step = 30,hour = 10,up_load=1)
